...
const AddEditCard: React.FC = () => {
  ...
  const labelClass = "block text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-1 font-orbitron";

  return (
    <div className="min-h-screen bg-[#0a0c10] py-12 px-4">
      <div className="max-w-4xl mx-auto tech-border tech-border-tl tech-border-tr tech-border-bl tech-border-br bg-slate-900/40 p-1 backdrop-blur-md">
        <div className="bg-slate-950/80 p-8">
          <div className="flex justify-between items-center mb-10 border-b border-blue-500/20 pb-4">
            <h2 className="text-2xl font-orbitron text-slate-100 tracking-widest">
              [ {id ? 'EDIT_ARCHIVE' : 'NEW_ENTRY'} ]
            </h2>
            {/* Wrap text with special characters in braces to avoid JSX parser issues */}
            <button onClick={() => navigate(-1)} className="text-slate-500 hover:text-blue-400 text-xs font-orbitron tracking-tighter transition-colors">
              {"<< RETURN"}
            </button>
          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative aspect-[3/4] bg-slate-950 border border-slate-800 flex items-center justify-center overflow-hidden group">
                  {formData.photo ? (
                    ...
                    <div className="text-center text-slate-700 font-orbitron text-[10px]">NO_DATA</div>
                  )}
            ...
            <div className="md:col-span-8 space-y-6">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input type="text" value={formData.eyeColor} onChange={(e) => setFormData(prev => ({ ...prev, eyeColor: e.target.value }))} className={inputClass} placeholder="BLUE" />
                </div>
                <div>
                  <label className={labelClass}>Hair Style</label>
                  <input type="text" value={formData.hairStyle} onChange={(e) => setFormData(prev => ({ ...prev, hairStyle: e.target.value }))} className={inputClass} placeholder="SHORT" />
                </div>
              </div>

              <div>
                <label className={labelClass}>Psychology / Personality</label>
              ...
              <div>
                <label className={labelClass}>Interests / Hobbies</label>
                <textarea rows={3} value={formData.hobbies} onChange={(e) => setFormData(prev => ({ ...prev, hobbies: e.target.value }))} className={inputClass} />
              </div>

              <div>
                <label className={labelClass}>Additional Logs</label>
                <textarea rows={2} value={formData.others} onChange={(e) => setFormData(prev => ({ ...prev, others: e.target.value }))} className={inputClass} />
              </div>
            </div>
          </div>

          <div className="mt-10 pt-6 border-t border-blue-500/10 flex justify-end">
            <button
              onClick={handleSave}
              className="px-10 py-2 bg-blue-600/20 border border-blue-500 text-blue-400 font-orbitron text-sm tracking-widest hover:bg-blue-600 hover:text-white transition-all shadow-[0_0_15px_rgba(59,130,246,0.3)]"
            >
              COMMIT_CHANGES
            </button>
          </div>
